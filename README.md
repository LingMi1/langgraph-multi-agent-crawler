...
A budget-triggered working-memory compaction (`compact_history`: keep system +
recent frames, fold the rest into one summary — LLM summarizer first, a rule
summarizer as fallback) answers the classic "how do you manage long
...
> 85-second full crawl (3 LLM calls); a 2-worker distributed scheduler
> batch-ran 3 sites with exactly-once execution; working-memory compaction
> keeps long FC loops inside context budget; golden success rate scores
> quality **and** LLM cost per task; all numbers reproducible offline via
