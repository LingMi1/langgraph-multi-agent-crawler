...
**覆盖率口径（诚实版）**：CI 门禁 30%，3.12 腿实测约 35%。测试火力集中在 agent 行为层（安全 / ReAct / 预算 / 熔断 / 压缩 / 评估指标）与工具、API、队列层；编排主文件 `graph/nodes.py`（5400+ 行）由图装配冒烟 + golden 端到端兜底，核心决策路由的单元测试正在渐进补齐。我更在意"哪些被测、哪些靠什么兜底"说得清楚，而不是把数字冲高。
